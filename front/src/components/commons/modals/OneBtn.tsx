...
const OneBtnModal: React.FC<ModalProps> = ({modalItems}) => {
  ...
  const contentHandler = () => {
    switch (modalType) {
      ...
      case "input" :
        return (
          <div className={`${containClass} flex-col gap-2`}>
            <div className="font-pre-B text-black text-xl w-full">{modalItems.title}</div>
            <input
              autoComplete="false"
              maxLength={10}
      ...
      case "ing" :
        return (
          <div className={`${containClass} flex-col gap-4 pt-1`}>
            ...
            {/* loading GIF */}
            <svg xmlns="http://www.w3.org/2000/svg" width={66} height={66} viewBox="0 0 24 24"><g fill="none" stroke="#007bff" strokeLinecap="round" strokeWidth={2}>
  ...
  const btnClass: string = "h-[35px] font-gtr-B text-lg text-white rounded-[15px]"
  const btnHandler = () => {
      ...
      case 1:
        return (
          <div className="flex w-full justify-center">
            <button className={`${btnClass} w-[130px] bg-main-blue`}>확인</button>
          </div>
        )
      ...
      case 2:
        return (
          <div className="flex gap-4 w-full justify-end">
            <button className={`${btnClass} w-[70px] bg-disabled-gray`}>취소</button>
            <button className={`${btnClass} w-[70px] ${modalType === 'warn' ? 'bg-red' : 'bg-main-blue' }`}>확인</button>
          </div>
        )
    }
  }
