...
import { PartyListData } from "../../types/Group";
...
const ListComponent:React.FC = () => {
  ...
  return (
    ...
    { partyList.map((item) => (
      <Link to={`/group/${item.id}`} key={item.id} className="relative w-100 h-20 rounded-2xl flex items-center border border-disabled-gray">
        <img src={item.partyProfile} className="w-14 h-14 rounded-full ms-4 me-5"/>
        <p className="text-xl font-pre-R">{item.partyName}</p>
        <img src={item.id === user.uploadGroupId ? favorStar : unFavorStar} className="absolute w-6 h-6 right-5"/>
      </Link>
    ))}
  </div>
  )
