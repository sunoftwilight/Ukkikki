...
import { InsertPasswordProps } from "../../types/GroupType";
...
const InsertPassword: React.FC<InsertPasswordProps> = ({
	...
	const handleInputChange = (
		...
		if (index < 6) {
			...
			setConfirmPassword(
				(prevConfirmPassword) =>
					prevConfirmPassword.substring(0, index - 6) +
					value +
					prevConfirmPassword.substring(index - 6 + 1),
			);
		}
	...
	const createRequest = async () => {
		...
		const file = new File([createData.partyProfile], "image.jpeg", {
			type: "image/jpeg",
		});
	...
	return (
		<div className="flex flex-col w-full h-full font-pre-B px-4">
			...
			<div className="w-full text-xl text-white">
				<button
					className="w-full h-[60px] bg-main-blue rounded-2xl"
					onClick={handleNextBtnClick}
				>
					<p>완료</p>
				</button>
				<button
					className="w-full h-[60px] bg-gray mt-2 rounded-2xl"
