...
import { CreatePartyData, CreateDoneProps } from "../types/GroupType";

const GroupCreate: React.FC = () => {
	const emptyFile: File = new File([], "");

	const [isState, setIsState] = useState<String>("info");
	const doneData = useRef<CreateDoneProps>({
		partyPk: null,
		partyName: "",
		inviteCode: "",
	});
	const createData = useRef<CreatePartyData>({
		partyName: "",
		partyPass: "",
		simplePass: "",
		partyProfile: emptyFile,
	});
	...
	return (
		<div className="w-full h-full p-4">
			{isState === "info" && (
				<InsertInfo
					onNextBtnClick={(type, data) => {
						insertDataChange(type, data);
					}}
			...
			{isState === "pass" && (
				<InsertPassword
					onBackBtnClick={(type, data) => {
						insertDataChange(type, data);
					}}
					onNextBtnClick={(type, data) => {
						doneDataChange(type, data);
					}}
					createData={createData.current}
